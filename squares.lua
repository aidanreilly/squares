...
function add_params()
	...
	for i = 1,16 do
		...
		params:add_control("pw" .. i, "pulse width " .. i, controlspec.new(0.0, 1.0, 'lin', 0.01, 0.5))
		params:set_action("pw" .. i, function(x) set_pw(i - 1, x) end)
...
function set_pw(synth_num, value)
	engine.pw(synth_num, value)
	edit = synth_num
	redraw()
end
...
function enc(n, delta)
	...
	elseif n == 3 then
		...
		elseif key_1_pressed == 0 and key_2_pressed == 0 and key_3_pressed == 1 then
			-- set the pw value
			params:set("pw" .. edit+1, params:get("pw" .. edit+1) + delta * 0.01)
...
function redraw()
	...
	screen.text(" p/w: ")
	screen.level(15)
	screen.text(params:get("pw" .. edit+1))
	screen.move(0,19)
	screen.level(2)
